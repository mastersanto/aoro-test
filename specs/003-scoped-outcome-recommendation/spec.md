...
- **At most one** bet-entry surface is mounted at any viewport, and at most one confirmation. Two containers exist — inline and sheet — but never both at once, and on a phone with the sheet dismissed there is legitimately none. *(Amended 2026-08-31 by feature 004 T12. This was enforced by counting elements carrying a dialog role; `004 UX-3` gives the bet sheet its own dialog role and accessible name, which a screen-reader user needs and which makes a bare role count read two. The count now identifies the confirmation by its accessible name — the thing this criterion is about — so it no longer conflates "a second dialog exists" with "a second confirmation exists". The criterion's meaning is unchanged and its enforcement is narrower: a duplicate confirmation still fails it, and a duplicate bet-entry surface is still caught by the separate outcome-group count.)*
