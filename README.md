...
**Status:** all seven features complete. 434 behaviour tests + 80 appearance checks + live tests passing.
...
Open the page. It opens **already on a market** — the most active one by 24-hour
volume — with a usable bet form, so you can start without hunting. Notice that it
picked the market and nothing else: no outcome is chosen and no amount is filled,
because those are the two decisions the project constitution reserves for you.

Twenty-four live Polymarket markets load below, each with its outcomes as implied
odds, 24-hour and total volume, and a resolution date.
